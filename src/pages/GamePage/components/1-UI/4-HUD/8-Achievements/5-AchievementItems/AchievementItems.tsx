...
const AchievementItems: FC = () => {
  ...
  const handleCalcItems = useCallback(() => {
    return Object.keys(activeAchievementObject)
      .map((item) => {
        return {
          id: nanoid(),
      ...
      .sort((a, b) => (a.value ? -1 : 1) - (b.value ? -1 : 1));
  }, [activeAchievementObject, handleItemAllColor, activeAchievementsModalTab]);

  // Local State
  const [items, setItems] = useState(handleCalcItems());

  const handleUpdateItems: ConstantVoidFunctionType = useCallback((): void => {
    setItems(handleCalcItems());
  }, [setItems, handleCalcItems]);

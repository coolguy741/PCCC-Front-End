...
type GLTFResult = GLTF & {
  ...
  materials: {
    ["VColors_C-M-S-R-AO"]: MeshStandardMaterial;
  };
};
...
const Kitchen = (props: JSX.IntrinsicElements["group"]) => {
  ...
  // Global State
  const { dynamicKitchen } = useGlobalState(
    (state) => ({
      dynamicKitchen: state.dynamicKitchen,
    }),
    shallow,
  );

  // Hooks
  const { enable, intensity, kitchenVisible, aoIntensity, colorMultiplier } =
    useControls({
      kitchen: folder(
        {
  ...
  const handleUpdateGLTFScene = useCallback(() => {
    if (!dynamicKitchen) return;

    loader.parse(dynamicKitchen, "", (gltf) => {
      if (gltf.scene) {
        gltf.scene.traverse((child) => {
          if (child instanceof Mesh || child instanceof SkinnedMesh) {
            if (child.name === "kitchen") {
  ...
  }, [dynamicKitchen, handleOBC, handleOBCAlpha, intensity, enable]);

  // Listeners
  useEffect(handleUpdateGLTFScene, [handleUpdateGLTFScene, dynamicKitchen]);

  return (
    <group visible={kitchenVisible} renderOrder={1}>
      {gltfScene ? (
        ...
        <mesh
          ...
          material={materials["VColors_C-M-S-R-AO"]}
